...
public class KeyValueStorageRocksDB implements KeyValueStorage {
    ...
    public KeyValueStorageRocksDB(String basePath, String subPath, DbConfigType dbConfigType) throws IOException {
        this(basePath, subPath, dbConfigType, false);
    }

    public KeyValueStorageRocksDB(String basePath, String subPath, DbConfigType dbConfigType, boolean readOnly) throws IOException {
        try {
            RocksDB.loadLibrary();
        } catch (Throwable t) {
    ...
    private RocksDB initializeRocksDBWithConfFile(String basePath, String subPath, DbConfigType dbConfigType, boolean readOnly, String dbFilePath) throws IOException {
    ...
    private RocksDB initializeRocksDBWithConf(String basePath, String subPath, DbConfigType dbConfigType, boolean readOnly) throws IOException {
...
//            int formatVersion = conf.getInt(ROCKSDB_FORMAT_VERSION, 2);
    ...
    public Entry<byte[], byte[]> getFloor(byte[] key) throws IOException {
        try (Slice upperBound = new Slice(key); ReadOptions option = new ReadOptions(optionCache).setIterateUpperBound(upperBound); RocksIterator iterator = db.newIterator(option)) {
            iterator.seekToLast();
            if (iterator.isValid()) {
